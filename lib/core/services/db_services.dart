...
class DatabaseServices {
  ...
  /// Formula Builder
  ///
  Future<bool> saveFormula(FormulaModel formula, userId) async {
    try {
      final ref = _db.collection('formulas').doc(); // unique doc ref

      formula.id = ref.id; // assign doc id to formula
      formula.userId = userId; // new field: userId

      await ref.set(formula.toJson());
  ...
  /// Get All Data
  ///
  Future<List<FormulaModel>> getAllFormulas(String currentUserId) async {
    try {
      final snapshot =
          await _db
              .collection('formulas')
  ...
  Future<void> updateFormulaSentStatus(String formulaId) async {
    await _db.collection('formulas').doc(formulaId).update({'sent': true});
    debugPrint('✅ Formula $formulaId sent status updated to true');
  }
  ...
  Future<List<ClientProfile>> getClientProfilesForUser(
    ...
    try {
      final snapshot =
          await _db
              ...
              .get();
      print("cleint data ==> ${snapshot.toString()}");
      print(
        "cleint data length getClientProfilesForUser==> ${snapshot.docs.length}",
      );
      return snapshot.docs
          .map((doc) => ClientProfile.fromJson(doc.data()))
          .toList();
    } catch (e, s) {
      debugPrint('Exception @getClientProfilesForUser: $e');
      debugPrint(s.toString());
      return [];
    }
  }
